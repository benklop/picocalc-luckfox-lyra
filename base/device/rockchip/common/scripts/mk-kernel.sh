...
do_make_kernel_config()
{
	...
	# Parse fragment arguments - handle both individual args and colon/space-separated strings
	FRAGMENT_ARGS=()
	for arg in "$@"; do
		# If argument contains colons or spaces, split it
		if [[ "$arg" == *":"* ]] || [[ "$arg" == *" "* ]]; then
			IFS=': ' read -ra SPLIT_FRAGMENTS <<< "$arg"
			FRAGMENT_ARGS+=("${SPLIT_FRAGMENTS[@]}")
		else
			FRAGMENT_ARGS+=("$arg")
		fi
	done
	...
	for fragment in "${FRAGMENT_ARGS[@]}"; do
